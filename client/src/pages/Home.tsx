...
export default function Home() {
  ...
  const recentOrders = summaryQuery.isError || !user ? orders : (ordersQuery.data ?? []).slice(0, 4).map((order: any) => ({
      ...
      <main className="lg:pl-[252px]">
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
          ...
          <div className="mb-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard label="Chiffre d'affaires" value={summary ? Number(summary.sales).toLocaleString("fr-FR") : "4 285 000"} suffix="FCFA" change="+12,8%" trend="up" accent="dark" />
            <StatCard label="Commandes" value={summary ? String(summary.orders) : "128"} change="+8,2%" trend="up" />
            <StatCard label="Produits en stock" value={summary ? summary.stock.toLocaleString("fr-FR") : "1 249"} change="-2,4%" trend="down" />
            <StatCard label="Nouveaux clients" value={summary ? String(summary.customers) : "36"} change="+18,6%" trend="up" />
          </div>
